...
Keep the conversation flowing naturally while being genuinely helpful.`,
    ...
    return (
      ...
      'Hey! Sorry about that, I got a bit distracted. What were you saying?'
    );
  } catch (error) {
...
router.get('/:id', auth, async (req: IUserRequest, res) => {
  try {
    ...
    if (!conversation) {
      console.log(
        `❌ Conversation ${req.params.id} not found or unauthorized for user ${req.user._id}`,
      );
    ...
    const response = {
      ...
      metadata: {
        created:
          conversationObj.metadata.created instanceof Date
            ? conversationObj.metadata.created.toISOString()
            : conversationObj.metadata.created,
        updated:
          conversationObj.metadata.updated instanceof Date
...
router.post('/', auth, async (req: IUserRequest, res) => {
  try {
    ...
    if (isCallSimulator) {
      ...
      const initialTitle = await generateConversationTitle([
        systemMessage,
        welcomeMessageWithAudio,
      ]);
...
Maintain a natural flow of conversation as if chatting with a friend while being genuinely helpful and taking direct responsibility for solutions.`,
    ...
    const casualGreetings = ['Hey there! ', 'Hi! ', 'Hello! ', 'Hey! ', 'Hi there! '];
...
For example, instead of "You'll need to contact our support team", say "I can help you with that right now" or "I'll take care of this for you."`,
      ...
      const aiResponse =
        completion.choices[0]?.message?.content ||
        'I can help you with that right away. Could you please repeat your request?';
...
- Account Access Problem`,
    ...
    const responseMessage = {
      ...aiMessage,
      timestamp:
        aiMessage.timestamp instanceof Date
...
router.get('/stats/user/:userId', auth, async (req: IUserRequest, res): Promise<void> => {
  try {
    ...
    const stats = {
      ...
      averageDuration: 0,
      lastActivity: conversations.length > 0 ? new Date(0) : null,
      sentimentBreakdown: {
        positive: 0,
        negative: 0,
        neutral: 0,
    ...
    if (conversations.length > 0) {
      ...
      const totalDuration = conversations.reduce(
        (sum, conv) => sum + (conv.metadata.duration || 0),
        0,
      );
      ...
      stats.lastActivity = lastConversation.metadata.updated;

      console.log(
        `📈 Average duration: ${stats.averageDuration}s, Last activity: ${stats.lastActivity}`,
      );
      ...
      conversations.forEach((conv) => {
        ...
        if (!sentiment || !['positive', 'negative', 'neutral', 'urgent'].includes(sentiment)) {
          sentiment = 'neutral';
          console.log(
            `⚠️ Invalid sentiment "${conv.metadata.sentiment}" for conversation ${conv._id}, defaulting to neutral`,
          );
        }
        ...
        console.log(
          `📝 Conversation ${conv._id}: ${messageCount} messages, sentiment: ${sentiment}, company: ${company}`,
        );
      });
